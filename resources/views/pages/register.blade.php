...
<section class="container" style="padding: 100px 0; background: #D9D9D9; min-height: 100vh; display: flex; align-items: center; justify-content: center;">
    <div style="width: 100%; max-width: 500px; background: white; border: 6px solid black; box-shadow: 15px 15px 0px black; padding: 40px;">
        ...
        <form action="{{ route('register.post') }}" method="POST" style="display: flex; flex-direction: column; gap: 20px;">
            @csrf

            @if($errors->any())
                <div style="border: 4px solid red; padding: 12px; background: #fff0f0;">
                    @foreach($errors->all() as $error)
                        <p style="font-weight: 900; font-size: 0.8rem; margin: 0;">{{ $error }}</p>
                    @endforeach
                </div>
            @endif
            ...
            <div>
                ...
                <input type="text" name="username" value="{{ old('username') }}" placeholder="IDENTITY_NAME" 
       style="width: 100%; border: 4px solid black; padding: 12px; font-family: inherit; font-weight: 900; outline: none; background: #f0f0f0;">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" placeholder="EMAIL_ADDRESS" 
                       style="width: 100%; border: 4px solid black; padding: 12px; font-family: inherit; font-weight: 900; outline: none; background: #f0f0f0;">
            </div>
