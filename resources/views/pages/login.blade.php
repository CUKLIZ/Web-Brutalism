...
<section class="container" style="padding: 100px 0; background: #D9D9D9; min-height: 100vh; display: flex; align-items: center; justify-content: center;">
    <div style="width: 100%; max-width: 500px; background: white; border: 6px solid black; box-shadow: 15px 15px 0px black; padding: 40px;">
       ...
       <form action="{{ route('login.post') }}" method="POST" style="display: flex; flex-direction: column; gap: 25px;">
    @csrf

    @if($errors->any())
        <div style="border: 4px solid red; padding: 12px; background: #fff0f0;">
            @foreach($errors->all() as $error)
                <p style="font-weight: 900; font-size: 0.8rem; margin: 0; color: red;">{{ $error }}</p>
            @endforeach
        </div>
    @endif
    ...
    <div>
        ...
        <input type="text" name="identifier" value="{{ old('identifier') }}" placeholder="ENTER_IDENTITY..." 
               style="width: 100%; border: 4px solid {{ $errors->has('identifier') ? 'red' : 'black' }}; padding: 15px; font-family: inherit; font-weight: 900; outline: none; background: #f0f0f0;">
    </div>

    <div>
        <label style="display: block; font-weight: 900; font-size: 0.8rem; margin-bottom: 8px;">[PASSWORD]</label>
        <input type="password" name="password" placeholder="••••••••" 
               style="width: 100%; border: 4px solid {{ $errors->has('password') ? 'red' : 'black' }}; padding: 15px; font-family: inherit; font-weight: 900; outline: none; background: #f0f0f0;">
    </div>
